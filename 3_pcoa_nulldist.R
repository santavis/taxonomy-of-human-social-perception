# Principal coordinate analysis (PCoA) of social perceptual features with permutation statistics in the primary dataset
...
for(i in seq(from=1,to=p)){
  ...
  # We know that only first 8 components are significant based on preliminary permutations so store the loadings only for these components
  ...
  if(i%%batch == 0){ # Save null distributions for every batch
    ...
    write.csv(weights_perm,paste("/path/pcoa/permutations/pcoa_components_nulldist_",i,".csv",sep = ""))
    write.csv(loadings_perm1,paste("/path/pcoa/permutations/pcoa_loadings_pc1_nulldist_",i,".csv",sep = ""))
    write.csv(loadings_perm2,paste("/path/pcoa/permutations/pcoa_loadings_pc2_nulldist_",i,".csv",sep = ""))
    write.csv(loadings_perm3,paste("/path/pcoa/permutations/pcoa_loadings_pc3_nulldist_",i,".csv",sep = ""))
    write.csv(loadings_perm4,paste("/path/pcoa/permutations/pcoa_loadings_pc4_nulldist_",i,".csv",sep = ""))
    write.csv(loadings_perm5,paste("/path/pcoa/permutations/pcoa_loadings_pc5_nulldist_",i,".csv",sep = ""))
    write.csv(loadings_perm6,paste("/path/pcoa/permutations/pcoa_loadings_pc6_nulldist_",i,".csv",sep = ""))
    write.csv(loadings_perm7,paste("/path/pcoa/permutations/pcoa_loadings_pc7_nulldist_",i,".csv",sep = ""))
    write.csv(loadings_perm8,paste("/path/pcoa/permutations/pcoa_loadings_pc8_nulldist_",i,".csv",sep = ""))
