...
#   3. Barplot PCoA loadings for main dimensions (Figure 4)
#   4. Correlation matrix of clusters (Figure 5)
#   5. Tsne plot (Figure 6)
#   6. Generalization to full movie data (Figure 7)
#        a) PCoA generalization plot (7a)
#        b) Clustering generalization (7b & 7c)
#   7. Generalization to movie frame data (Figure 8)
#        a) PCoA generalization plot (8a)
#        b) Clustering generalization (8b)
#   8. Demographical factors and social ratings (Figure 10)
#
...
## 3. Barplot PCoA loadings for significant components (Figure 4)
...
for(i in seq(from=1,to=8)){ # Plot only significant components (first 8)
  ...
  #png(file = paste("/path/Fig4_pcoa/barplots/",i,"_component_name.png",sep = ""),width = 1000,height=h[i])
  ...
  #png(file = paste("/path/Fig4_pcoa/barplots/",i,"_component.png",sep = ""),width = 1000,height=h[i])
  p <- ggplot(df1,aes(x=d,y=load,fill=sign)) +
    geom_bar(stat = "identity") +
    scale_fill_identity() +
    ylim(-1,1) +
...
#cormat
#pdf(file = "/path/Fig5_clustermatrix/materials/cormat_megaperception.pdf",width = 22,height=20)
...
#cormat lower
#pdf(file = "/path/Fig5_clustermatrix/materials/cormat_lower_megaperception.pdf",width = 22,height=20)
...
#consensus upper 
#pdf(file = "/path/Fig5_clustermatrix/materials/consensus_upper_megaperception.pdf",width = 22,height=20)
...
#pdf(file = "/path/Fig5_clustermatrix/materials/consensus_30rect_megaperception.pdf",width = 22,height=20)
...
##---------------------------------------------------------------------------------------------------------------------------------------------------------------
# 5. TSNE plot for concordance analysis (Figure 6).
...
#pdf("/path/Fig6_tsne/tsne_numbered_dots_new.pdf",width = 8,height = 8)
ggplot() +
...
#pdf("/path/Fig7_generalization/materials/pcoa_generalization.pdf",width = 8,height = 8)
...
##---------------------------------------------------------------------------------------------------------------------------------------------------------------
# 6b. Clustering generalization plot, alluvial & correlation matrices (Figure 7b & 7c)
...
# Plot alluvial
#pdf("/path/Fig7_generalization/materials/validation_alluvial.pdf",width = 20,height = 20)
...
#pdf("/path/Fig7_generalization/materials/validation_alluvial_rectangles.pdf",width = 20,height = 20)
ggplot(alluvial_data,aes(y = rep(1,76),axis1 = clips_clusters, axis2 = movie_clusters_flip)) +
...
#pdf("/path/Fig7_generalization/materials/consensus_megaperception_k16.pdf",width = 22,height = 20)
...
#pdf("/path/Fig7_generalization/materials/consensus_movie_k19.pdf",width = 22,height = 20)
...
pdf("/path/Fig7_generalization/materials/cormat_megaperception.pdf",width = 22,height = 20)
...
#pdf("/path/Fig7_generalization/materials/cormat_movie.pdf",width = 22,height = 20)
...
##---------------------------------------------------------------------------------------------------------------------------------------------------------------
# 7a. PCoA generalization from movie clips to movie frames (Figure 8a) 
...
# Plot the results
cc <- ifelse(grepl("Clips", colnames(corReorder(cormat,order = "hclust",hclust_type = "average"))), "#2166AC", "#B2182B")
#pdf("/path/Fig8_framegeneralization/materials/pcoa_generalization.pdf",width = 8,height = 8)
...
##--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 8. Correlate each participants responses to the mean of all others (ethnicity % age related features).
...
ggplot(output, aes(x = age, y = r)) +
  ...
  geom_smooth(method = "lm", se = FALSE) +  # Adding regression line
...
ggplot(output_sex, aes(x = r_type, y = r)) +
  ...
  labs(y = "Rating similarity with others (r)", x = "Sex") +
  theme_minimal()
#dev.off()
