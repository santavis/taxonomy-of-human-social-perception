...
#   2. Stability of average measures ICC in emotion features (emotion data not publicly available)
#   3. Stability of single measures ICC in social features 
#   4. Stability of average measures ICC in social features 
#
#---------------------------------------------------------------------------------------------------------------------------------------------
...
# Path to save data
path <- "/path/icc_analysis_emotion/results"

# Load data
data_wide <- read.csv('/path/icc_analysis_emotion/data/megalocalizer_full_data_wide.csv') # emotion data not publicly available
...
#---------------------------------------------------------------------------------------------------------------------------------------------
# 2. Stability of average measures ICC in emotion features 
...
for(i in seq(from=7,to=length(dims))){
  ...
  # Create folder
  dir.create(paste(path,"/",dims[i-6],sep = ""))
  dir.create(paste(path,"/",dims[i-6],"/distributions",sep = ""))
  ...
  for(j in seq(from=2, to=min_subs)){ # Number of subjects in a set
    ...
    if(is.na(max_perm)){ # Very large number of possible combinations
      p <- perm
    } else {
      if(max_perm > perm){ # Very large number of possible combinations
        p <- perm
    ...
    for(l in seq(from=1, to=p)){ # permutations
      ...
      s <- m[,ind_col]
      res <- ICC(s,alpha=.05)
      perm_lb <- c(perm_lb,res$results$`lower bound`[5])
      perm_icc <- c(perm_icc,res$results$ICC[5])
      perm_ub <- c(perm_ub,res$results$`upper bound`[5])
    }
    
    # Save permuted distributions
    df <- data.frame(perm_icc,perm_lb,perm_ub)
    write.csv(df,paste(path,"/",dims[i-6],"/distributions/",j,"_subjects_average_measures.csv",sep = ""))
  ...
  df <- data.frame(dim_icc_lb,dim_icc_ub,dim_ci_lb,dim_ci_ub,dim_perms)
  write.csv(df,paste(path,"/",dims[i-6],"/results_average_measures.csv",sep = ""))
}
...
for(i in seq(from=1, to=length(data))){ # Dimensions
  ...
  for(j in seq(from=2, to=min_subs)){ # Number of subjects in a set
    ...
    # Sample random sets of subjects from each videoset for ICC caclulation
    v1_set <- combn(seq(from=1,to=idx[1]),j)
    v2_set <- combn(seq(from=idx[1]+1,to=idx[2]),j)
    v3_set <- combn(seq(from=idx[2]+1,to=idx[3]),j)
    v4_set <- combn(seq(from=idx[3]+1,to=idx[4]),j)
    ...
    # Save permuted distributions
    df <- data.frame(perm_icc,perm_lb,perm_ub)
    write.csv(df,paste(path,"/",dim_read[i],"/distributions/",j,"_subjects_average_measures.csv",sep = ""))
  ...
  df <- data.frame(dim_icc_lb,dim_icc_ub,dim_ci_lb,dim_ci_ub,dim_perms)
  write.csv(df,paste(path,"/",dim_read[i],"/results_average_measures.csv",sep = ""))
}